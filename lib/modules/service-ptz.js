...
const setPresetShema = Joi.object({
	ProfileToken: Joi.string().required().description("profile token"),
	PresetToken: Joi.string().optional().description("preset token"),
	PresetName: Joi.string().optional().description("preset name"),
});

/**
 * 5.4.1 SetPreset in [ONVIF-PTZ-Service-Spec](https://www.onvif.org/specs/srv/ptz/ONVIF-PTZ-Service-Spec.pdf) 
 * 如果 PresetToken 省略,那么设备会创建一个新的 preset.否则它会更新之前存储的位置和 name(如果提供的话).
 * 如果创建成功,reponse 中会携带 PresetToken,作为 preset 的唯一识别码.已经存在的 preset 可以通过 PresetToken 来指定并被覆盖.
 * 无论覆盖还是创建,可选项 PresetName 可以被指定.如果 PTZ 设备正在移动会导致 SetPreset 操作失败.设备可以在preset 中,内部保存一些
 * 额外状态,比如:图像属性等.这些状态,会在 GotoPreset 操作时被唤起调用.
 * 
 */
...
OnvifServicePtz.prototype.setPreset = function (params, callback) {
	let promise = new Promise((resolve, reject) => {
		const { error } = setPresetShema.validate(params);
		if (error)
			reject(error);
		let soap_body = '';
